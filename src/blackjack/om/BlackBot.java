 ...
 *         --- les gains sont placés dans miseJoueurs[j] (Ã  la place des mises)
 ...
 * 
 * sabot       : le sabot des cartes (mais dans une partie normale on n'a pas Ã  y toucher)
 * etat        : l'état de la partie MISE, JEU ou GAIN
 * mainBanque  : la main du croupier
 * mainJoueurs : tableau des mains des joueurs (mÃªme ceux qui ne participent pas Ã  un tour)
...
public class BlackBot {
	 ...
	 * Utilisable uniquement en état MISE si le joueur n'a pas déjÃ  "perdu" ou "terminé"
	 * 
	 * @param   joueur   numéro du joueur [0..nbJoueur-1]
	 */
	public void terminer(int joueur) {
		if ( this.etat==EtatBlackBot.JEU ) {
			if (this.finJoueurs[joueur]==false) {
				...
				throw new BlackjackException("[BlackBot] : Impossible de terminer sa main quand elle est déjÃ  terminée");
			}
		} else {
	...
	/**
	 * Lancer une nouvelle partie et distribuer les premiÃ¨res cartes
	 * Utilisable uniquement en etat MISE si au moins un joueur a misé
	 * 
	 */
	public void distribuer() {
		// Il faut Ãªtre en état MISE
		if (this.etat!=EtatBlackBot.MISE) {
	...
	/**
	 * Méthode interne (lancée automatiquement aprÃ¨s chaque tirage ou demande de fin de tour)
